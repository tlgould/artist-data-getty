...
#returns all variant names of a given ulan id
def get_getty_artist_var_names(ulan):
	#getty_url= 'http://vocab.getty.edu/sparql?query=DESCRIBE++%3Chttp%3A%2F%2Fvocab.getty.edu%2Fulan%2F'+ulan+'%3E&_implicit=false&implicit=true&_equivalent=false&_form=%2Fsparql'
	url = "http://vocab.getty.edu/ulan/%s.json" % ulan
	result = requests.get(url)
	data = json.loads(result.content)
	...
	for binding in bindings:
		if binding['Predicate']['value']== 'http://www.w3.org/2000/01/rdf-schema#label':
			names.append(binding['Object']['value'])
			
	return names

def get_artist_place_of_birth_death(ulan):
	url = "http://vocab.getty.edu/ulan/%s.json" % ulan
	result = requests.get(url)
	data = json.loads(result.content)
